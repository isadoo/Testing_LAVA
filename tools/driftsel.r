...
function(gridx, gridy, maxpar=6){
	...
	num = which(errormat==min(errormat,na.rm=TRUE))
...
function(whistory, maxk, maxpar=30){
	...
	num = which(likemat==max(likemat,na.rm=TRUE))
	ai = as.integer(grdim*(num/grdim - floor(num/grdim)))
	if( ai==0 ){ ai = grdim }
	bi = ceiling(num/grdim)
	a = arange[ai]
...
function(U, mux, sdx, signx){
	...
	if( max(vals,na.rm=TRUE)==-Inf ){ vals = rep(-1.0e-07, 10^4) }
	else{ vals[which(vals==-Inf)] = min(vals[which(vals>-Inf)]) }
	if( min(vals,na.rm=TRUE)==Inf ){ vals = rep(1.0e-07, 10^4) }
